...
| `microsoft-graph` | Microsoft Graph | [microsoftgraph/msgraph-metadata](https://github.com/microsoftgraph/msgraph-metadata) | Authoritative v1.0 and beta routes pinned and generated by UndREST-SpecQL |
...
ambiguous slugs remain literal. During matching, retained opaque identifiers are
compared against authoritative placeholders only when a unique least-general
route exists; ambiguous matches fail closed. Lookalike hosts are not treated as
Graph.

The bundled `microsoft-graph` pack contains 47,451 authoritative v1.0 and beta
routes generated by UndREST-SpecQL from pinned Microsoft metadata. It is loaded
locally and lazily only when needed; APISpy performs no runtime specification
fetch. Disable the pack from **Packs** to return Graph requests to the safe
...
│   └── RESEARCH_ARCHITECTURE.md # Research schemas, trust boundaries, and ownership
...
│   ├── lib/                     # filtering, matching, enrichment, and research modules
│   ├── icons/
│   ├── manifest.json
