...
APISpy has bounded request-side readiness for a future Microsoft Graph pack:

- the built-in `microsoft-graph` normaliser is registered through
  `Normalizer.registerPackNormaliser`;
- only HTTPS requests to the exact global host `graph.microsoft.com` are
  claimed;
- `/v1.0` and `/beta` remain explicit path segments and are also reported in
  the existing `apiVersion` field;
- GUID, numeric, and email-like identifier segments are templated, while
  ambiguous slugs remain literal;
- query parameter names remain available through the original URL.

There is no bundled Graph shard today. Real route, operation, version, auth, or
schema classification requires an authoritative generated SpecQL export. Do not
hand-author Graph shards or route metadata in APISpy, and do not add a runtime
fetch: generated packs remain checked-in offline data owned by the SpecQL export
pipeline.

The generic loader can discover a non-ARM shard by exact request host when
exactly one enabled shard advertises that host. It deliberately returns no
match for zero or multiple host candidates. If an authoritative Graph export
contains multiple shards for `graph.microsoft.com`, extend its generated
routing metadata and loader contract rather than choosing a shard
heuristically.
...
Microsoft Graph does not need a separate script include: its bounded built-in
normaliser is registered through this same extension point from
`lib/normalizer.js`. A future authoritative Graph pack should use the pack ID
`microsoft-graph`.
...
- [ ] Shard files generated by an authoritative SpecQL export (never hand-authored)
- [ ] Shard files use a currently supported grouped schema version
