...
identifier segments. Ambiguous path slugs remain literal; the matcher substitutes
opaque segments only when the authoritative shard yields a unique least-general
route, otherwise it fails closed.

The bundled `microsoft-graph` pack contains 47,451 authoritative v1.0 and beta
routes generated by UndREST-SpecQL from pinned Microsoft metadata. The existing
exact-host fallback loads its single shard lazily and locally; APISpy performs no
runtime specification fetch. Disabling the pack returns Graph requests to the
safe **No spec match** behavior.
...
python3 scripts/prepare_data.py --zip inventory/api-index-sharded.zip
```
...
# Validate prepare-data merge behavior and checked-in pack provenance/content
...
- **Additional authoritative non-ARM route data.**
  Microsoft Graph v1.0 and beta are bundled. Other platforms still require
  authoritative SpecQL-generated packs; APISpy does not fabricate route
  templates or fetch specifications at runtime.
...
2. **Additional API packs** — bundle other authoritative SpecQL exports through the existing pack pipeline.
